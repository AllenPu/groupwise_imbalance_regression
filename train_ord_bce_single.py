...
parser.add_argument('--ord_single', type=bool, nargs='*', default=True, help='train  with the mode of ordinary regression with single output')
parser.add_argument('--cls', type=bool, nargs='*', default=False, help='train  with the mode of ordinary regression only for cls')
parser.add_argument('--output_dim', type=int, default=1, help='number of out put dim')
...
def test_step(model, test_loader, device):
    ...
    for idx, (inputs, targets, group) in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            #
            ord_out = ord_out.view(bsz, -1)
            #
            ord_hat = torch.sum(ord_out, dim=-1)
            #
            print(" ord hat ", ord_hat.shape)
            print(" group ", group.shape)
            print(" ord out ", ord_out.shape)
            #
            # write down the acc
            assert ord_hat.shape == group.shape
            #
            acc_bs = torch.sum(ord_hat == group)/bsz
            #
