...
from network import ResNet_regression, ResNet_ordinal_regression, ResNet_regression_split
...
parser.add_argument('--regulize', type=bool, nargs='*', default=False,
...
def get_dataset(args):
    ...
    train_dataset = IMDBWIKI(data_dir=args.data_dir, df=df_train,
                             img_size=args.img_size, split='train', group_num=args.groups)
...
def test_step(model, test_loader):
    ...
    for idx, (inputs, targets, group) in enumerate(test_loader):
        ...
        with torch.no_grad():
            g_hat, y_hat = model(inputs.to(torch.float32))
            #
...
if __name__ == '__main__':
    ...
    #oss_or = nn.MSELoss()
    #
    model = ResNet_regression_split(args).to(device)
    ...
    '''
    #
    print(" tau is {} group is {} lr is {}".format(
        args.tau, args.groups, args.lr))
    #
